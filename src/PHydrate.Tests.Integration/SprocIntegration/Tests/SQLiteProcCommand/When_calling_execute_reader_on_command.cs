...
namespace PHydrate.Tests.Integration.SprocIntegration.Tests.SQLiteProcCommand
{
    ...
    public sealed class When_calling_execute_reader_on_command : SQLiteProcCommandSpecificationBase
    {
        private Establish Context = () => BaseCommand.Expect( x => x.ExecuteReader() );

        private Because Of = () => ProcCommand.ExecuteReader();
